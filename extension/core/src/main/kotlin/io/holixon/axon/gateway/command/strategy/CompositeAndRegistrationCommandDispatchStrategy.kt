 ...
 * making this decision by applying a logical AND to the decisions made by provided child strategies.
 * If the list of child strategies, remote registration delivers true.
 */
class CompositeAndRegistrationCommandDispatchStrategy(
  ...
  override fun registerRemote(commandName: String, messageHandler: MessageHandler<in CommandMessage<*>?>): Boolean {
    return if (childStrategies.isEmpty()) {
      ...
      childStrategies.all { child -> child.registerRemote(commandName, messageHandler) }
    }
  }
}